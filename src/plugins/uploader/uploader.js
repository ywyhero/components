...
const Uploader = (options = {}) => {
    ...
    let max_file_size = Number(options.max_file_size);
    let conf = {
        ...qiniuConf,
        ...options,
        // 此处禁用插件自带的自动上传
            ...
            'FilesAdded': function (up, files) {
                ...
                if (fileList.length > 0) {
                    if (fileList.length > maxFiles) {
                        ...
                        Message({ message: `文件过多，最多上传${maxFiles}个文件`, type: 'error' });
                        return;
                    }
